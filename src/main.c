...
char* expand_single_token(const char* token) {
    ...
    while (token[i] != '\0') {
        if (token[i] == '$') {
            ...
            size_t start = i;
            while (token[i] != '\0' && (isalnum((unsigned char)token[i]) || token[i] == '_')) {
                ++i;
            }
            size_t var_len = i - start;
            
            if (var_len > 0) {
                char* env_var = (char*)malloc(var_len + 1);
                if (env_var == NULL) {
                    perror("malloc failed");
                    exit(EXIT_FAILURE);
                }
                strncpy(env_var, token + start, var_len);
                env_var[var_len] = '\0';
...
int run(char** tokens, size_t size) {
    ...
    if (handle_builtin(tokens, size, &builtin_status)) {
